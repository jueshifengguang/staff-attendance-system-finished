...
<body>
	...
	uswMap = (HashMap<String,Object>)request.getAttribute("uswMap");
	String startTime = uswMap.get("work_start").toString();
	String endTime = uswMap.get("work_end").toString();
	String sHour = startTime.substring(0,2);
	String sMinute = startTime.substring(3,5);
	String eHour = endTime.substring(0,2);
	String eMinute = endTime.substring(3,5);
	
	Integer stHour = Integer.parseInt(sHour);
	Integer stMinute = Integer.parseInt(sMinute);
	Integer etHour = Integer.parseInt(eHour);
	Integer etMinute = Integer.parseInt(eMinute);
      ...
      <li class="nav-item">
        ...
        <div id="collapsePages" class="collapse" aria-labelledby="headingPages" data-parent="#accordionSidebar">
          <div class="bg-white py-2 collapse-inner rounded">
            ...
            <a class="collapse-item" href="director/selectAllShiftWork?s_id=<%=usr_id%>&usr_id=<%=usr_id %>&dep_id=<%=dep_id%>&select_year=<%=year%>&select_month=<%=month%>">个人工作班次信息</a>
	          ...
	          <div class="col-lg-7">
	            <div class="p-5">
	            	<div class="text-center">
	              		 <h1 class="h4 text-gray-900 mb-4">修改工作班次</h1>
	              		 <h3 class="h6 text-gray-900 mb-4"><%=uswMap.get("work_date") %></h3>
	           		</div><!--  text-center-->
	           			<form class="user" action="director/updateShiftWork"   method="post">
									
									<input type="hidden" name="usr_id" value="<%=usr_id%>"/>
									<!-- 隐藏域-----传sw_id -->
									<input type="hidden"   name="sw_id"   value="<%=uswMap.get("sw_id")%>"/>
									<!-- 当前界面员工显示情况 -->
									<input type="hidden"   name="s_id_now"   value="<%=uswMap.get("s_id_now")%>"/>
									<!-- 隐藏域-----传原来的起止时间 -->
									<input type="hidden"   name="work_date"   value="<%=uswMap.get("work_date")%>"/>
									<input type="hidden"   name="work_start"   value="<%=uswMap.get("work_start")%>"/>
									<input type="hidden"   name="work_end"   value="<%=uswMap.get("work_end")%>"/>
								
								
								<div class="row">
									<div class="col">
										<!-- 隐藏域----员工id -->
										员工ID：<%=uswMap.get("s_id")%>
									</div>
									<div class="col" >
										<!-- 隐藏于----日期 -->
										员工姓名：<%=uswMap.get("s_name") %>
									</div>
								</div>	
								...
								<div class="row">
									<div class="form-group col">
										开始时间：时
										<input type="text"  name="start_hour" value="<%=stHour%>"  class="form-control form-control-user Wdate" onkeyup="shuru(this);" oninput="if(value>23)value=23;if(value<0)value=0"/>
									</div>
									<div class="form-group col">
										&nbsp;
										 分
										<input type="text"  name="start_minute" value="<%=stMinute %>"  class="form-control form-control-user Wdate" oninput="if(value>59)value=59;if(value<0)value=0"/>
									</div>
								</div>	
								...
								<div class="row">
									<div class="form-group col">
										结束时间：时
										<input type="text"  name="end_hour"   value="<%=etHour%>" class="form-control form-control-user Wdate"  onkeyup="shuru(this);" oninput="if(value>23)value=23;if(value<0)value=0"/>
									</div>
									<div class="form-group col">
										&nbsp;
										 分
										<input type="text"  name="end_minute"   value="<%=etMinute%>" class="form-control form-control-user Wdate"  oninput="if(value>59)value=59;if(value<0)value=0"/>
									</div>
								</div>	
